...
if [ -n "$(.utils/plume-scripts/is-ci.sh)" ]; then
  num_jobs=1
else
...
if [ -n "${SKIP_JAVADOC+x}" ]; then
  ...
  if [ -f SKIP-REQUIRE-JAVADOC ]; then
    ...
    if ! ".utils/plume-scripts/ci-lint-diff" "/tmp/$USER/ap-warnings.txt"; then
      status=1
      reason="$reason
target 'api-private' failed"
    ...
    if ! ".utils/plume-scripts/ci-lint-diff" "/tmp/$USER/rj-warnings.txt"; then
      status=1
      reason="$reason
