...
public class FilterTest {
  ...
  /**
   * @param mainClass class that defines a main() that reads from System.in and writes to System.out
   * @param testInput test input for the filter
   * @param expected expected test output
   */
  public FilterTest(String description, Class mainClass, InputStream testInput, Reader expected) {
    super(description);
    ...
    try {
      _mainMethod = mainClass.getMethod("main", new Class[] {String[].class});
    } catch (NoSuchMethodException e) {
      throw new IllegalArgumentException(mainClass.getName() + " does not define main()");
    }
    if (!Modifier.isPublic(_mainMethod.getModifiers())
        || !Modifier.isStatic(_mainMethod.getModifiers()))
      throw new IllegalArgumentException(mainClass.getName() + " main() is not public and static");
  }

  /**
   * Runs the main() method in this.mainClass, piping this.testInput into its standard input.
   * Standard output is captured and compared to this.expected line-by-line.
   *
  ...
  protected void runTest() throws Throwable {
    ...
    ByteArrayOutputStream outBuffer = new ByteArrayOutputStream(INITIAL_BUFFER_SIZE);
    ByteArrayOutputStream errBuffer = new ByteArrayOutputStream(INITIAL_BUFFER_SIZE);
    PrintStream testOutput = new PrintStream(outBuffer, true); // autoflush
    PrintStream testError = new PrintStream(outBuffer, true); // autoflush
    ...
    BufferedReader actual = new BufferedReader(new StringReader(outBuffer.toString()));
  ...
  protected void assertEofMatch(int lineNum, String expected, String actual) {
    ...
    fail("mismatch on line " + lineNum + ": expected " + expected + ", but was " + actual);
  }

  /**
   * Compares expected and actual output lines. Asserts that lines are equal and reports the line
   * number, expected, and actual values otherwise. Subclasses should override to get specific
